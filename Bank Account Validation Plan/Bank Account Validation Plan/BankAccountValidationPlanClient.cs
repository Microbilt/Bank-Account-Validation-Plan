...
{
    public class BankAccountValidationPlanClient
    {
        public string Authorization { get; private set; }
        public readonly ABAAcctVerificationClient ABAAcctVerificationClient;
        ...
        public BankAccountValidationPlanClient(string client_id, string client_secret, EnvironmentType type = EnvironmentType.Production)
        {
            baseUri = new Uri(type.ToDescriptionString());
            Authorization = MBAuthorizations(client_id, client_secret);
            ABAAcctVerificationClient = new ABAAcctVerificationClient(Authorization, baseUri);
        }
            ...
            using (var client = new HttpClient() { BaseAddress = baseUri })
            {
                ...
                try
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        {
                            throw new Exception(string.Format("Authorizations failed : {}", response.Content.ReadAsStringAsync().Result));
                        }
                    }
                    else
                    {
